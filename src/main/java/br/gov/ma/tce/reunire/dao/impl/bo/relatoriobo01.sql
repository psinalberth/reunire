select
	'TOTAL (VIII) = (VI + VII)' total, subtotal, total_categoria, categoria, origem, especie, sum(val_pin) val_pin, sum(val_pat) val_pat, sum(val_rre) val_rre 
from (
select
	'SUBTOTAL COM REFINANCIAMENTO (VI) =  (IV + V)' subtotal, 'SUBTOTAL DAS RECEITAS (IV) = (I + II + III)' total_categoria,
...
select 'SUBTOTAL COM REFINANCIAMENTO (VI) =  (IV + V)' subtotal, 'SUBTOTAL DAS RECEITAS (IV) = (I + II + III)' total_categoria, 
	   'Recursos Arrecadados em Exercícios Anteriores (III)' categoria, '' origem, '' especie, null val_pin, null val_pat, null val_rre

union all

select
	'SUBTOTAL COM REFINANCIAMENTO (VI) =  (IV + V)' subtotal, '' total_categoria, 'Operações de Crédito/Refinanciamento (V)' categoria, 
...
(case 
    ...
    when categoria = 'Operações de Crédito/Refinanciamento (V)' then 4
end),
(case 
